...
export class FaucetChecker {
  ...
  private async checkFaucetMaxRewardsPerWeek(): Promise<{ error: boolean, msg: string }> {
    ...
    let count                                 = 1;
    let txCount                               = 1;
    while (count > 0 && txCount > 0) {
      const { transactions, count: c } = await this.api.transactions.getList({
        limit           : 1,
        "and:senderId"  : faucet.rewardAddress,
        "and:type"      : TransactionType.SEND,
        orderBy         : "height:asc",
        "and:fromHeight": startHeight
      });
      count                            = parseInt(`${c}`);
      txCount                          = transactions.length;
      allTransactions.push.apply(allTransactions, transactions);
      if (transactions.length > 0) {
        startHeight = transactions[transactions.length - 1].height + 1;
      }
    }
